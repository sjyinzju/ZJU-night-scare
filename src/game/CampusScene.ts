...
export class CampusScene extends Phaser.Scene {
  ...
  private resolveMovement(input: InputVector, stepDistance: number) {
    ...
    if (onRoad && nearest) {
      ...
      // Hysteresis: if the locked direction is still available, require a
      // clear threshold before switching. This prevents flicker / jitter
      // at symmetric junctions where two branches score identically.
      let best = options[0];
   ...
   * Includes forward / reverse on the current segment, plus any
   * segment starting from a junction node within range.
   */
  ...
  private resolveWalkablePoint(point: IsoPoint) {
    ...
    // Snap to the nearest road. Roads are always walkable — do NOT
    // check isBlocked on snapped points; roads may pass near water.
