...
export interface StoryResultOption {
  id: string;
  text: string;
  title?: string;
  body: string[];
...
export interface StoryChoice {
  ...
  text: string;
  next: StorySceneId;
  /** Optional in-place narrative beat shown before the story advances to the next scene. */
  result?: {
    title?: string;
    body: string[];
    highlights?: string[];
    continueText?: string;
    choices?: StoryResultOption[];
  };
...
export interface StoryScene {
  ...
  body: string[];
  /** A few authored warning lines rendered separately as flashing red text. */
  highlights?: string[];
  choices: StoryChoice[];
  effect?: HorrorEffect;
...
export const storyScenes: Record<StorySceneId, StoryScene> = {
  library_intro: {
    ...
    setting: "indoor",
    body: [
      "　　2008 年 3 月初，夜里十点。农医馆缩在校园最偏僻的西南角，离宿舍区很远，平时就少有人来。今晚馆内只坐着稀稀拉拉二十几个学生，暖气开得很足，窗户全部关着，玻璃外连树枝摇动的声音都传不进来。你陪室友林伟来这里自习，本来只听得见翻书、落笔和偶尔压低的咳嗽声，头顶的灯却不知从什么时候起染上了一层暗红，像隔着一层洗不净的旧血。",
    ...
    choices: [
      {
        ...
        result: {
          title: "被划掉的名字",
          body: [
            "　　你把笔记本挪到手电光最亮的地方，从第一页开始逐行核对。被划掉的不只是林伟的名字：每隔几页都会出现一条相同格式的记录，左边是日期，右边是姓名。日期彼此相隔七年，姓名却都被人用黑笔反复涂抹，纸面留下近乎相同的凹痕，像同一只手在不同年份做过同一件事。",
            "　　越靠近最后一页，墨迹越新。最末一行的日期正是今晚，姓名栏却仍然空着；你盯着它时，空格左端慢慢渗出一滴新墨。右侧小隔间里的打印机随之转了一圈，滚轴声停下后，纸页深处又响起一下很轻的抓挠。你可以继续辨认旧名字，也可以先检查正在变化的空格。",
          ],
          highlights: ["3 月 8 日，23:47。　借阅人：＿＿＿＿＿＿"],
          choices: [
            {
              id: "read-buried-name",
              text: "迎着手电斜光，辨认没有被划净的旧名字",
              title: "墨痕下面的人",
              body: [
                "　　你让光束贴着纸面掠过，黑墨下的笔画被凹痕一点点托起。最早那条记录只能辨出一个姓，后面的名字几乎全烂在水渍里；直到第三条，你才在涂黑的边缘拼出两个娟秀的字。那不是林伟的笔迹，更像有人先写下名字，又逼着他将它划掉。",
                "　　你默念出那两个字时，夹页里的湿发突然收紧，纸张啪地合拢了一半。右侧终端同时亮起一道冷白反光，断电的打印机开始缓慢进纸。那张新票也许能解释这个名字为何一次次出现在七年的间隔里。",
              ],
              highlights: ["苏　婉"],
              continueText: "前往借阅终端",
            },
            {
              id: "watch-empty-name",
              text: "盯住今晚的空白姓名栏，看墨迹会写出什么",
              title: "正在填写的空格",
              body: [
                "　　新墨沿纸纤维向右爬行，先勾出一道横，又落下一笔竖。那笔画像极了你的姓，可写到一半，笔记本里忽然传来翻页声；前面的纸页明明没有动，最末一页的空格却像被一只看不见的手按住，继续向下凹陷。",
                "　　你立刻移开视线。未写完的字停止生长，右侧小隔间却传来同样的落笔节奏——咔、咔、咔。那不是笔尖，而是断电打印机的字头正在撞击纸面。再等下去，姓名栏或许真的会被填满。",
              ],
              highlights: ["借阅人：张——"],
              continueText: "立刻前往借阅终端",
            },
          ],
        },
      ...
      },
      {
        ...
        result: {
          title: "镜头里多出的一行",
          body: [
            "　　快门声在空荡的阅览区里显得格外刺耳。手机刚保存照片，屏幕便自行把最后一页放大；原本空白的页脚多出一行极淡的小字，字的边缘带着液晶屏坏点般的红光。你重新低头，纸面上仍然什么也没有。",
            "　　你把手机与笔记本并排举起。屏幕里的文字没有消失，反而在每次自动对焦后更清楚一点；照片右上角还映出一张你拍摄时没有看见的白纸，正从右侧小隔间的打印机里缓慢吐出。现实中的隔间随即传来进纸摩擦声，像照片里的那台机器先动了一步。",
          ],
          highlights: ["右侧终端。先看日期，再看纸背。"],
          continueText: "前往借阅终端",
        },
      ...
      },
      {
        ...
        result: {
          title: "合上之后",
          body: [
            "　　你合上笔记本，用掌心压住自行翘起的封皮。里面那阵若有若无的呼吸立刻停了，夹页中的湿发也不再滑动；可金属搭扣始终扣不上，每次压下去，都会被里面什么东西轻轻顶开。",
            "　　桌边的暗红随之熄灭，右侧小隔间却亮起一线冷白。断着电的打印机开始空转，一张新吐出的纸正擦过滚轴；空椅也朝隔间转了半圈，像替你指出唯一还能确认的东西。你没有再翻开笔记本，但它留在掌下的温度正在一点点升高。",
          ],
          continueText: "前往借阅终端",
        },
  ...
  library_receipt: {
    ...
    body: [
      ...
      "　　票面最上方印着『农医馆一层』，借阅人一栏完全空白，连供填写姓名的横线也像被人仔细擦掉了。索书号 R-1953 指向一份早已封存的戏曲病理档案，归还地点却写着『医学院地下仓库』；你从未在馆内指示牌上见过这个地点。打印时间是 23:47，日期正好与林伟笔记本的最后一页相同。更奇怪的是，票据下方列着三次续借记录，日期分别相隔七年，借阅人始终为空，而每次经办人的签名都是林伟的笔迹。最后一次续借时间，就是今晚。",
      "　　你把小票与笔记本并排放在桌上，最显眼的是那个重复出现的时间：23:47。票纸背面并不平整，斜着迎向手电时能看见层层叠叠的压痕；最深的几笔旁边，还粘着细碎的黄色纸屑和香灰。你可以继续核对时间与笔迹，也可以借手电辨认压痕，或者先离开这个越来越不对劲的隔间。",
      "　　隔板外忽然响起纸页被逐张翻动的声音。每翻一页，声音便向入口挪近一点；可从隔板下方看出去，地上没有鞋，也没有影子。最后一声翻页停在你背后，空椅自行转了半圈，扶手上缓慢浮出一道湿漉漉的五指印。紧接着，左侧书架旁的桌面传来一声极轻的纸响，像有人刚把什么东西放在那里。黑暗中亮起下一处猩红微光，那道红光旁边，还有一双刚刚踩出的湿鞋印。",
    ],
    choices: [
      {
        id: "compare-time",
        ...
        result: {
          title: "同一个 23:47",
          body: [
            "　　你把小票压在笔记本最后一页旁，从日期、分钟到数字的写法逐项核对。两处的 7 都多出一道短横，4 的收笔也向左微勾；这不是打印字体，而是机器把某个人的手写笔迹原样压进了票面。",
            "　　三条续借记录彼此相隔七年，却全部停在 23:47。最后一条写着今晚，下一行则只有尚未打印完整的浅灰色方框。小票不像在记录已经发生的借阅，更像在沿着固定间隔等待下一次重复。你还可以继续比对经办人的笔迹，或算清七年间隔指向什么。",
          ],
          highlights: ["23:47　／　23:47　／　23:47"],
          choices: [
            {
              id: "compare-handwriting",
              text: "把经办人签名与林伟笔记逐笔叠在一起",
              title: "不是预言",
              body: [
                "　　你将两张纸贴在手电前，签名与笔记里的每一处停顿都严丝合缝，连最后一笔因恐惧而产生的颤抖也完全重合。唯一说不通的是：票上的字压得更深，像林伟在写笔记以前，打印机就已经记住了他的手。",
                "　　纸张重叠的瞬间，背面最深的一组压痕被光照透。四个字从黄色纸屑间显出来，随即烫得你松开手。碎屑落向隔板外，在左侧桌边连成一条断续的线。",
              ],
              highlights: ["经办人：林伟"],
              continueText: "循着黄色纸屑寻找旧符",
            },
            {
              id: "count-seven-years",
              text: "按七年的间隔倒推最早一次续借日期",
              title: "早于这座图书馆",
              body: [
                "　　你在笔记边缘倒推三次日期：2008、2001、1994，间隔确实都是整整七年。可最早两条记录出现时，紫金港校区尚未建成，更不可能存在票面写着的『农医馆一层』。这张票记录的也许不是某座建筑，而是一条后来被新校园覆盖的旧路。",
                "　　你再看小票，最早日期旁已经浮出一小块潮湿泥印，下面还粘着香灰与黄色纸屑。纸屑从指间飘落，全部向隔板外同一处红光滑去，像被那里的一张旧符吸住。",
              ],
              highlights: ["第一次续借：1994 年 3 月 8 日，23:47"],
              continueText: "循着黄色纸屑寻找旧符",
            },
          ],
        },
      ...
      },
      {
        ...
        result: {
          title: "纸背的警告",
          body: [
            "　　你让手电贴着纸面横照，原本杂乱的凹痕逐渐分成三行。它们并不是同一次留下的：第一行笔画平稳，第二行已经开始发抖，第三行则压得几乎穿透纸面；每一行的末端，都粘着一点不同深浅的香灰。",
            "　　光束移动时，三行字不会同时显现。你只能先把光停在其中一处：最上面的短句、中间那行反复加重的警告，或最深处那四个几乎戳破纸背的字。隔板外的翻页声越来越近，你没有时间逐字慢慢读完。",
          ],
          highlights: ["不要回答。"],
          choices: [
            {
              id: "read-lake-warning",
              text: "先辨认最上方那句较完整的字",
              title: "第一行压痕",
              body: [
                "　　最上方的字迹终于连成一句完整的话。你读到最后一个字时，终端黑屏里掠过一片倒置的湖面；水中站着一个白衣女人，她背对着你，头却在倒影里缓慢转了过来。",
                "　　你立刻压低小票。幻影消失，最深处的凹痕却同时发热，四个字旁的黄色纸屑纷纷脱落，沿桌沿飘向隔板外的猩红微光。那里像有某种东西正替你挡住逐渐靠近的翻页声。",
              ],
              highlights: ["湖边不要回头。"],
              continueText: "循着黄色纸屑寻找旧符",
            },
            {
              id: "read-name-warning",
              text: "追着反复加重的笔画读第二行",
              title: "第二行压痕",
              body: [
                "　　第二行有几个字被反复描了三遍，像书写者担心后来的人看不清。你刚辨出『名字』，书架深处便有人用林伟的声音叫了你一声；那声音很近，近得仿佛嘴唇正贴着隔板上缘。",
                "　　你咬住牙没有应答。声音停下后，最深一行的黄色纸屑忽然发热，一粒粒落向隔板外的左侧桌面。那里的猩红微光下，隐约压着一张烧去一角的旧符。",
              ],
              highlights: ["听见有人唱你的名字，不要回答。"],
              continueText: "循着黄色纸屑寻找旧符",
            },
            {
              id: "trace-deepest-warning",
              text: "用指腹沿最深的四个字慢慢描过去",
              title: "最深的一行",
              body: [
                "　　你不用看清，指腹已经先认出了那四个凹下去的字。最后一笔几乎刺穿票纸，孔洞里塞着香灰；当你描到末端，纸背突然传来一下微弱的脉动，像这张票在你手里跳了一次。",
                "　　紧接着，嵌在笔画里的黄色碎屑全部脱落，越过桌沿向隔板外飘去。它们没有落地，而是在黑暗中排成一条细线，尽头正是左侧桌面那点猩红微光。",
              ],
              highlights: ["符在外面。"],
              continueText: "循着黄色纸屑寻找旧符",
            },
      ...
      },
      {
        ...
        result: {
          title: "口袋里的余温",
          body: [
            "　　你没有继续辨认压痕，把小票沿原有折痕收进口袋，立刻退到隔板外。空椅在你离开后自行转回终端，椅背上那道湿手印也随之消失；打印机却又吐出半寸空白票纸，像还想把你叫回去。",
            "　　口袋里的余温没有散去，反而隔着衣料越来越烫。几粒粘在纸背的黄色碎屑从袋口漏出，落地后没有停下，而是被一阵只贴着地面的风推向左侧桌面。那里刚亮起一点猩红微光，光下压着一张烧去一角的旧黄符。",
          ],
          continueText: "去左侧桌面拿取旧符",
        },
  ...
  library_shelf: {
    ...
    choices: [
      {
        id: "pull-book",
        text: "抽出旧书确认后方无人，再循湿脚印追出去",
        next: "library_fall",
        result: {
          title: "书架后面没有墙",
          body: [
            "　　你扣住发潮的书脊，猛地将旧书整个抽出。后方没有人，也没有本该存在的墙，只有一片刚好容得下一张脸的黑暗；冷风从洞里向外吹，带着湖水与旧香灰混在一起的气味。两点浑浊的眼白在深处睁开，位置比正常人的眼睛高出整整一层书架。",
            "　　口袋里的旧符骤然亮了一下，焦黑边缘又烧去一小块。那双眼睛随即退远，黑暗像被人从后面合上，只剩一张借阅卡飘落在地。卡上林伟名字的下一行已经写出你的姓；你还没伸手，纸页便啪地闭合，门口那串湿脚印也重新向闸机外延伸。",
          ],
          highlights: ["下一位借阅人：张——"],
          continueText: "循湿脚印追出去",
        },
      ...
      {
        id: "light-gap",
        text: "用手电照清白衣退去的方向，循湿脚印追出去",
        next: "library_fall",
        result: {
          title: "白衣退去的方向",
          body: [
            "　　你抬高手电，让光束穿过书架最宽的一道缝。另一侧先露出一截湿透的白袖，接着是垂到腰间的黑发；它始终背对着你，却在光里沿通道无声倒退。每退一步，地面才迟半拍浮出一枚鞋印，像脚印并不属于那件白衣。",
            "　　光束追到通道尽头时，白衣在闸机旁突然变薄，像一张被雨水泡透的纸贴进门缝。湿脚印没有消失，继续越过出口，指向停车场另一侧；林伟的声音也从同一方向传来，反复催你快一点。你无法确认那是不是他，但至少看清了它们离开的方向。",
          ],
          continueText: "循湿脚印追出去",
        },
        requireItem: "flashlight",
      ...
      {
        id: "listen-back",
        text: "后退辨清脚步通向出口，再跟上去",
        next: "library_fall",
        result: {
          title: "离开的脚步",
          body: [
            "　　你没有再碰那本书，只闭住呼吸向后退。木板另一侧的脚步仍比你慢半拍：你退一步，它便跟一步；你停住，它也停住。第三次后退时，那脚步却没有跟随，而是突然从书架另一端越过你，沿通道一路奔向出口。",
            "　　每响一步，门口便多出一枚朝外的湿鞋印。脚步穿过闸机以后，书架内侧又响起一次与你完全同步的呼吸——这里或许还有另一个东西没有离开。最后一声脚步落在雨里，林伟贴着夜色喊你快来，声音里夹着一段极轻的女声。",
          ],
          highlights: ["跟我来。不要回头。"],
          continueText: "循湿脚印追出去",
        },
  ...
  library_fall: {
    ...
    strongBlur: true,
    body: [
      "　　你跟着那双湿鞋印冲出书架房间。脚印穿过闸机，越过停车场，一直伸向医学院教学楼。林伟的声音在前方反复叫你跟上，可雨幕里始终看不见他的身影。等你追到教学楼下，三楼中间那扇窗正敞着，窗帘被风吹得翻出墙外；楼前空地被暴雨前的黑暗吞得只剩轮廓，远处路灯只留下一根模糊的影子。就在你抬头寻找林伟时，歌声突然停在最高的那个音上。砰——巨响与贴脸的白影同时撞进视野，像有什么沉重的东西从高处砸穿雨幕。你的耳朵里只剩尖锐蜂鸣，胸口也被震得发麻；下一道惨白闪电掠过夜空，整片空地在一瞬间亮得没有阴影。",
  ...
  library_police: {
    ...
    setting: "indoor",
    body: [
      "电话接通后，你强迫自己把医学院教学楼的位置和林伟的情况说清。保卫处让你留在原地，不要移动现场；120 的接线员一遍遍确认他的呼吸和伤势。无论你刚才先靠近、先观察还是先拨号，能做的事很快都只剩在雨里等待。二十分钟后，救护车、保卫处和警车先后赶到，医学院教学楼下拉起了警戒线。红蓝灯在雨幕中一阵阵扫过，把地上的水痕切成短促的明暗。",
      "一位姓张的警官带你去做笔录。问了一个多小时。做完后他放了一段监控录像：画面里，林伟走到三楼窗户前，站了半分钟，翻身跳了下去。但画面的角落里，模模糊糊出现了一双脚——穿着绣花鞋，裙摆遮住了脚踝。那个身影站了几秒，转身消失了。",
      "「警官，监控最后那个……那双脚是怎么回事？」张警官的目光微微闪烁了一下。「光线反射的问题，我们有经验。」「可那明明是一双绣花鞋啊！」「同学，案件结论是林伟因学业压力过大自杀身亡。这件事就到此为止。不要再追问，也不要在学校里传播未经证实的说法。」",
      "他话里有话。你盯着他的眼睛，忽然明白了：他知道那不是光线反射，只是不肯说。一个警察用官方结论堵住你的嘴，说明这件事的盖子远比你想象的重。林伟最后留下的警告、监控里的绣花鞋和口袋里发烫的小票都在提醒你：事情远没有结束。",
      "离开笔录室时，手机亮了一下。白秋发来一条消息：『我在白沙楼下等你。别再回头看医学院。』你从未告诉她自己来了这里。雨声敲在走廊尽头的窗上，你决定先回白沙，问清她为什么知道。",
    ],
    choices: [
      {
        id: "return-dorm",
        text: "带着监控里的疑点回白沙宿舍，问清白秋为什么知道",
        next: "dorm_baiqiu",
  ...
  dorm_baiqiu: {
    ...
    setting: "outdoor",
    body: [
      "从农医馆出来时，雨已经把校园洗成一层发亮的黑。借阅小票贴在湿透的口袋里，那张从封条后取下的符纸隔着衣料一阵阵发烫。直到白沙宿舍的门禁灯出现在雨幕里，你才发现整片宿舍区安静得反常：没有外卖车，没有说话声，本该亮着的窗口也像被谁逐层按灭。你刚抬起校园卡，身后忽然有人叫你的名字。",
      "白秋站在路灯与树影交界处，穿着那件熟悉的白色羽绒服，手里的伞却偏向你这一侧。她平时很少显出慌乱，此刻脸色却比灯下的雨雾还白。她没有问你去了哪里，只盯着你沾灰的袖口说：你还是进了那栋楼。远处闷雷滚过，她看向白沙高层最黑的那扇窗，随后快步挡在门禁前。",
      "你把林伟坠楼、借阅终端的最后记录和书架后的封条告诉她。听见那段若有若无的戏腔时，她握伞柄的手明显收紧。白秋说，林伟不是第一个。前几届也有人在午夜听见女人唱名：有人在启真湖边醒来，鞋里全是水；有人忘掉整整一晚，只留下走向医学院的定位；还有人回宿舍后反复说，夜里的走廊比白天多出了一段，第三天便休学失踪。",
      "你问她为什么知道这些。白秋沉默很久，才说自己小时候就听过同一首歌，那时她甚至还没来浙大。家里老人告诉她，有些声音并非从某处传来，而是在寻找能听见它的人；一旦回应名字，它就会借你的记忆认路。紫金港后来扩建，旧路被新楼盖住，可被埋掉的只是路面，不是曾在上面行走的东西。",
      "白秋忽然按住你的衣袋。图书馆里取得的符纸隔着布料烫得惊人，边角已经焦黑。她没有再给你第二张符，只让你把现有的符贴身收好。她认得这种画法：这不是普通护身符，而是一道被撕下来的封条。它能替你挡住一次靠近，却也说明原来镇住的东西已经少了一角。你拿走它时，也许已经让某种东西记住了你的气息。",
      "门禁机突然发出通过声，可你们谁都没有刷卡。玻璃门自行弹开几厘米，带着铁锈味的冷风从缝里涌出。门后没有值班灯，只有更深处一截猩红的光忽亮忽灭。白秋压低声音：看见寝室门开着，不要立刻进去；走廊的灯若开始一盏盏亮，也别把它当作指路。它只是在确认你走到了哪里。",
      "她又叮嘱你，无论听见室友、宿管，还是她本人的声音，都先看对方脚下有没有影子。不要回应从身后叫出的名字，不要数楼梯，也不要在雷声之后看窗玻璃。符纸发热时绝不能松手。她不能陪你进去，因为里面的东西认识她；只要她跨过门槛，今晚白沙所有寝室门都会一起打开。",
      "林伟留下的线索就在宿舍里，论坛旧帖、失踪学生与医学院封条之间还缺最后一环。白秋看出你不会回头，便把伞塞进你手中，自己站回雨里：进去以后，别相信白沙给你看的第一个出口。如果符纸完全变黑，就什么都不要找了，立刻逃。猩红的光从门缝爬到你的鞋尖，她隔着雨说：张超，答应我，至少活着回来。",
    ],
    choices: [
      {
        id: "promise",
        text: "答应她会活着回来，接过伞进宿舍",
        next: "dorm_forum",
        statChanges: { sanity: 3, trust: 15, clues: 3 },
      },
      {
        id: "press",
        text: "听出她仍在隐瞒，不再逼问，记下疑点后进宿舍",
        next: "dorm_forum",
  ...
  baiqiu_umbrella: {
    ...
    body: [
      ...
      "走到宿舍拐角，她忽然停下，把你装着符纸的手按回衣袋，说：如果哪天我不见了，别去湖边找我，也别去小剧场。你握住她冰凉的手，说你不会让那一天来。她没有反驳，只是把脸别过去，肩膀轻轻抖了一下。",
  ...
  medical_entry: {
    ...
    body: [
      ...
      "教学楼共六层，廊灯全灭。门口贴着一张被雨水泡皱的施工告示，落款是三年前；盖在上面的保卫处封条却很新，裂口也新，像刚有人从里面出来。",
      "门禁屏没有显示一至五层，只有“6”亮着。旁边的疏散图更奇怪：所有箭头都从顶层开始，一层一层向下，最后钻进一块被黑笔涂掉的地下区域。像有人提前替你排好了调查顺序。",
      "你抬头看见林伟坠落的那扇顶层窗。窗框里像垂着一缕长发；眨眼后，只剩被风吹歪的窗帘。与此同时，楼内传来病床轮子滚过地面的声音——声音却来自你的头顶。",
    ],
    choices: [
      {
        id: "front",
  ...
  medical_garage: {
    ...
    body: [
      "你沿楼梯一层层向下。五层、四层、三层的标牌都被刮花，平台和过渡走廊却干净得像刚拖过。每下一层，消毒水味就重一点，病床轮子的声音也近一点；可当楼层牌从“2”跳到“B1”时，你清楚地记得自己多走了一段楼梯。",
      "地下一层豁然展开成停车场。混凝土柱把黑暗切成一格一格，停车位和行车通道向多个方向延伸。林伟那晚走的是相反方向——从这里一路向上，最后到了你刚检查过的窗。",
      "水泥柱后传来轮子滚动的声音。你把手电照过去，光圈里只有空车位和一根还在晃的铁链；地上却多了两道新鲜轮痕，正好接上六层窗边那串鞋印。",
      "柱上的疏散图也从顶层画起，红线穿过车库，终点仍被黑笔涂掉。你走到哪一根柱子，身后的回声就停在哪一根柱子——它没有跟着你，它是在等你。",
    ],
    choices: [
      {
        id: "follow-route",
        text: "沿轮痕穿过停车场，寻找地下仓库",
        next: "medical_vault",
  ...
  medical_window: {
    ...
    deferInInterior: true,
    body: [
      "门在顶层打开。你先闻到白墙和旧地胶之间残留的消毒水味，随后才看见那条很长的教学走廊：连续的房门、相似的转角、三间加装防盗门的教室，一直延伸到视线尽头。",
      "林伟坠落的窗在走廊中段，仍开着一条缝。窗框有抓痕，暖气片后的监控线被剪断，地上留着未干的鞋印。奇怪的是，鞋尖没有朝向窗外，而是从窗边转身，整齐地走进了向下的楼梯间。",
      "玻璃映出你和身后第二个人。你猛地回头，走廊空着；再看玻璃，第二道影子已经站到下一层的平台。短短两三秒的戏腔从楼下传来，唱到一半就被病床轮子的吱呀声盖住。",
      "楼梯扶手上系着一截发黑的红绳，和被剪断的监控线缠在一起。红绳的另一端垂向楼下，像在给你指路。",
    ],
    choices: [
      {
        id: "check-window",
        text: "记下窗框、鞋印和红绳的对应关系，再下楼",
        next: "medical_garage",
        statChanges: { sanity: -3, clues: 8 },
  ...
  medical_vault: {
    ...
    body: [
      "轮痕穿过停车场，停在一扇没有写进疏散图的铁门前。门禁卡在锁槽里响了一下；农医馆小票上的归还地点，原来就是这间封存的地下仓库。",
      "里面比车库更密、更冷。柜体、旧设备和窄通道挤在一起，病床与解剖台被隔帘切成一间间看不全的暗室。每张床腿都缠着发黑的红绳——材质和六层窗边那一截完全相同。",
      "你在铁柜里找到一本 1953 年名册，索书号与空白借阅记录对上：R-1953。夹页里还有一张旧设备通道图，路线从这间仓库继续通向小剧场后台；地图上被涂黑的终点终于有了名字。",
      "墙角摆着一双鞋尖朝墙的旧绣花鞋。鞋底沾着沼泽地的泥，花纹与六层湿鞋印一致。仓库最里面则留着一块空底座，像刚移走过很重的东西；窗边、车库、仓库一路出现的水迹，在底座前停住，又沿墙面反重力地向上流。",
    ],
    choices: [
      {
        id: "read-roster",
  ...
  stand_ground: {
    ...
    setting: "indoor",
    body: [
      "你没有跑。那个女人看了你很久，脸上的表情从戒备慢慢变成了某种接近释然的东西。她先指向头顶——六层窗的方向；再沿你走过的楼梯、车库轮痕和红绳逐一指回身边的名册与空底座；最后才指向小剧场。她的身体像雾气一样散去，地上多了一张泛黄的老照片。",
